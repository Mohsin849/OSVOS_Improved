...
def main(seq):
    ...
    max_training_iters = 5000       # Change this according to the model name if using the pretrained models
    ...
    annotation_imgs = [cv2.imread(img,0) for img in annotation_filenames] #read annotations 

    og_img_filenames = glob.glob(os.path.join(og_img_path, '*.jpg'))
    og_img_filenames.sort()
    og_img = [cv2.imread(img) for img in og_img_filenames]  #Origional Images
